...
ostream &User::PrintAddress(char * print, std::ostream & out) {
    for (int i = 0; i < strlen(print); ++i) {
        if (print[i] != UNDERLINE)
            out << print[i];
        else
            out << SPACE;
    }
...
void User::Fix_Name(char * res) {
    bool check = true;
    for (int i = 0; i < strlen(res); ++i){
        if (check && Check_Lower(res[i])) {
            res[i] = Lower_To_Upper(res[i]);
            check = false;
        }
        else if (!check && Check_Upper(res[i]))
            res[i] = Upper_To_Lower(res[i]);
        else if(res[i] == UNDERLINE)
            check = true;
        if (i == 0)
            check = false;
        else if(res[i-1] == UNDERLINE)
            check = false;
    }
}

char *User::CheckAddress() {
    char loc[MAX];
    bool check;
    do {
        cout << "Enter an address, up to 30 characters:" << endl;
        msg06();
        cin.getline(loc, MAX);
        ...
        for (int i = 0; i < len; ++i) {
            if (!Check_Number(loc[i]) && !Check_Let(loc[i]) && loc[i] != SLASH && loc[i] != SPACE &&
                loc[i] != BACKSLASH) {
...
bool User::Set_Birthday() {
    ...
    bool check;
    do {
        cout << "Enter the year, 1900 is the minimum and 2010 is the maximum." << endl;
        cin.getline(year, MAX);
    } while (!Check_Date(year, 2010,1899));
    do {
...
ostream& operator<<(ostream& out, User& print){
    out << "ID: " << print.Get_ID() << endl;
    ...
    out << "Address: ";
    print.PrintAddress(print.Get_Address(), out);
